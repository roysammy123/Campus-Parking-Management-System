...
def random_datetime(date):
    return datetime(date.year, date.month, date.day, random.randint(8, 18), random.randint(0, 59), random.randint(0, 59))
...
for i in range(num_days):
    ...
    for _ in range(entries_per_day):
        ...
        # Add the data entry
        data.append([vehicle_number, in_time.strftime('%d-%m-%Y %H:%M:%S'), out_time.strftime('%d-%m-%Y %H:%M:%S'), available_slot])
